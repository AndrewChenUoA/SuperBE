...
wrlist = ["Category", "Sequence", "File", "TP", "TN", "FP", "FN", "Rec", "Spec", "FPR", "FNR", "PWr", "F-Meas", "Prec", "Mask Err", "N", "R", "DIS", "numMin", "phi", "POST or TargetSeg"]
    ...
    with open (directory+result_file, 'r') as csvfile:
        ...
        if overalls:
            ...
            try:
                ...
                agg_write.writerow((row[0],"OVERALL","OVERALL",sums[0],sums[1],sums[2],sums[3],means[0],means[1],means[2],means[3],means[4],means[5],means[6],str(2),row[15],row[16],row[17],row[18],row[19]))
    csvfile.close()
